...
table2 <- kable(medication_table, booktabs = T, row.names = NA, format = "latex", caption = "Medication") %>%
  kable_styling(full_width = TRUE) %>%
  row_spec(0,bold=TRUE)
...
For comparison, we estimate model systems with and without cross-model covariance. With cross-model covariance estimated, the model corresponds to seemingly unrelated model system, and without the model is Bayesian network with separated local distributions. This simpler model is estimated first with only potassium and phosphorous concentrations as responses.
...
dialdiet_gamma_mv2_epros <- mebn.bipartite_multivariate(reaction_graph = initial_graph, 
                                   ...
                                   local_model_cache = "models/BLMM_gamma_qr_multivariate2/fat_epros", 
                                   stan_model_file = "mebn/v2/BLMM_gamma_qr_mv.stan",
                                   normalize_values = TRUE)

write.graph(dialdiet_gamma_mv2_epros, "graphs/dialysis_gamma_multivariate2_epros.graphml", "graphml")

```

For getting stricter recommendations, we add plasma albumin concentration as a third constraint. 

First, estimate the model without missing values in P-Alb. The rows with missing values are held out from the density estimation, but patients are kept in the model for estimating their parameters.

```{r pk_fppi_palb_targets}
...
dialdiet_gamma_mv3_missing_palb <- mebn.bipartite_multivariate(reaction_graph = initial_graph, 
                                   inputdata = dialysis,
                                   ...
                                   group_column = "potilas",
                                   local_estimation = mebn.multivariate_sampling,
                                   local_model_cache = "models/BLMM_gamma_qr_multivariate3/fat_epros_missing_palb", 
...
```{r, eval=TRUE, warning=FALSE, cache=TRUE, eval=FALSE}
...
for (person_id in 1:length(patients_with_missing_palb)) {
  ...
  local_distributions$modelcache <- "models/BLMM_gamma_qr_multivariate3/fat_epros_missing_palb"
  
  # - get personal data, normalized and original
  
  # - statistics for vertex levels are calculated from normalized data (as it was in the likelihood estimation)
  predictors <- nrow(assumedpredictors_fat_epros)
  normalized_input <- sapply(1:predictors, mebn.scale_gaussians, data = dialysis, datadesc = assumedpredictors_fat_epros)
  ...
  personal_data_org <- subset(dialysis[dialysis$potilas == subject_code,], select = as.vector(assumedpredictors_fat_epros$Name))
  ...
  personal_graph <- mebn.extract_personal_graph_from_mv(person_id, initial_graph, personal_model_dir, assumedpredictors_fat_epros, pk_fppi_palb_targets, "models/BLMM_gamma_qr_multivariate3/fat_epros_missing_palb", personal_data, personal_data_org, datadesc_fat_epros)
}
...
for (i in 1:nrow(rows_with_missing_palb)) {
  ...
  print(paste0("Reading personal data imputation model '", personal_model_dir, "/personal_graph.graphml'"))
...
With this data imputation we can estimate a cross-covariance model with three responses
...
dialdiet_gamma_mv3 <- mebn.bipartite_multivariate(reaction_graph = initial_graph, 
                                   ...
                                   group_column = "potilas",
                                   local_estimation = mebn.multivariate_sampling,
                                   local_model_cache = "models/BLMM_gamma_qr_multivariate3/imputed_palb", 
                                   stan_model_file = "mebn/v2/BLMM_gamma_qr_mv_cv.stan",
                                   normalize_values = TRUE)

write.graph(dialdiet_gamma_mv3, "graphs/dialysis_gamma_multivariate3_imputed.graphml", "graphml")
...
dialysis_imputed <- dialysis_imputed[order(dialysis_imputed$hoitoryhma, dialysis_imputed$potilas, dialysis_imputed$havainto),]
...
dialdiet_gamma_mv3_two_level <- mebn.bipartite_two_level_multivariate(reaction_graph = initial_graph, 
                                   inputdata = dialysis_imputed,
                                   targetdata = no_holdout,
                                   predictor_columns = assumedpredictors_fat_epros, 
                                   assumed_targets = pk_fppi_palb_targets, 
                                   ...
                                   local_model_cache = "models/BLMM_gamma_mv_cross/two_levels", 
                                   stan_model_file = "mebn/v2/BLMM_gamma_two_level_grouping.stan",
                                   normalize_values = TRUE)

write.graph(dialdiet_gamma_mv3_two_level, "graphs/dialysis_gamma_two_level_grouping.graphml", "graphml")

```


```{r dialysis_gamma_mv_cross_2levels_ppc, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE, cache=TRUE, fig.cap="In this model the three concentrations and their parameters are stacked into one univariate model for estimating cross-model correlations."}
...
```{r, effect_correlation_plot, fig.height = 15, fig.align="left", cache=TRUE}
library(rstan)
library(ggcorrplot)
...
C_extract <- extract(target_blmm, pars = c("C_s"))
...
colnames(C_cross.df) <- c("P-K level", paste("P-K: ", as.vector(assumedpredictors_fat_epros$Description), sep = ""), "fP-Pi level", paste("fP-Pi: ", as.vector(assumedpredictors_fat_epros$Description), sep = ""), "P-Alb level", paste("P-Alb: ", as.vector(assumedpredictors_fat_epros$Description), sep = ""))
rownames(C_cross.df) <- c("P-K level", paste("P-K: ", as.vector(assumedpredictors_fat_epros$Description), sep = ""), "fP-Pi level", paste("fP-Pi: ", as.vector(assumedpredictors_fat_epros$Description), sep = ""), "P-Alb level", paste("P-Alb: ", as.vector(assumedpredictors_fat_epros$Description), sep = ""))

p <- nrow(assumedpredictors_fat_epros) + 1

ggcorrplot(C_cross.df, hc.order = FALSE, lab = FALSE, type="lower") +
  scale_fill_gradient2(limit = c(min(C_cross),max(C_cross)), low = "blue", high = "red", mid = "white", midpoint = mean(C_cross)) +
  ggplot2::labs(x = NULL, y = NULL) +  
  ggplot2::ggtitle("Cross-model correlations of personal effects") +
  ggplot2::theme(
...
```{r, top_correlations, cache=TRUE}
effect_cor.df <- data.frame(matrix(ncol = 3, nrow = 0), row.names = NULL)

for (x in 1:dim(C_cross.df)[1])
{
  for (y in 1:dim(C_cross.df)[2])
  {
    eff <- C_cross.df[x,y]
    from <- colnames(C_cross.df)[x]
    to <- rownames(C_cross.df)[y]
...
kable(top_correlations, "latex", escape = F, booktabs = T, row.names = FALSE, digits = 3, caption = "Table shows 30 highest positive or negative correlations between personal effects of nutrients or other considered predictors. This structure of correlations is used in estimating the personal effects based on personal intake and matching concentrations.")

```



##Effects of nutrition in both treatment and personal levels

First, we create personal models that include also information of personal treatment
...
model_dir_candidates <- c("mv3_cross_two_levels", "mv3_cross", "mv3")
...
saveRDS(mse_summary, "model_evaluation/NRMSE_comparison_cross_12.rds")

write.csv2(mse_summary, file="model_evaluation/NRMSE_comparison_cross_12.csv", row.names = FALSE)


```
```{r MSE_table}

mse_summary <- readRDS("model_evaluation/NRMSE_comparison_cross_12.rds")
...
mse_table <- rbind(mv3c2, mv3c1)
mse_table[,2:5] <- round(as.numeric(mse_table[,2:5]),3)

kable(mse_table, "html", escape = F, booktabs = T, row.names = FALSE, digits = 3,align="lrrrr",
  caption = "Personal graphical models with different reaction model candidates are evaluated with NRMSE for each modelled concentration and average model error.",
 ...
 kable_styling(latex_options="scale_down") %>%  
  ...
  #  save_kable(file = "tables/model_nrmse_comparison.pdf", keep_tex = TRUE)
...
Then, we make a summary from effects of treatment and personal variations within the treatment
...
colnames(effect_summary) <- c("person_id", "effect", "general_effect", "general_effect_lCI", "general_effect_uCI", "sigma_g", "sigma_g_lCI", "sigma_g_uCI", "sigma_b", "sigma_b_lCI", "sigma_b_uCI", "treatment_group", "group_effect", "group_effect_lCI", "group_effect_uCI", "personal_effect","personal_effect_lCI","personal_effect_uCI")

saveRDS(effect_summary, "patient_summary/two_level_effect_summary.rds")

```

# Two level effect summary

```{r effect_summary_table, cache=TRUE}
library(dplyr)
...
Following table summarizes the personal effects of potassium and phosphorous intake on concentrations. 

```{r pot_pho_effects_only, cache=TRUE, echo=FALSE}
...
This large table in includes all estimated nutrition effects 

```{r full_effects_table, cache=TRUE, echo=FALSE}
#two_level_effects_table.df <- two_level_effects_table.df[two_level_effects_table.df$abs_general > 0.2,]

#two_level_effects_table.df <- two_level_effects_table.df[two_level_effects_table.df$effect_description == "Act. D-vit -> P-Alb",]
...
# variations removed 
kable(two_level_effects_table.df[c(3:4,5,8:16)], "latex", escape = F, booktabs = T, row.names = FALSE, digits = 2,align="llccccccccccc",
  caption = "Variation of nutrition effects between different dialysis treatments and between patients within the same treatment.",
  col.names = c("Nutrient", "Conc.", "General effect", "avg", "min", "max", "avg", "min", "max", "avg", "min", "max")) %>%
  column_spec(1, width = "7em") %>%
  column_spec(2, width = "4em") %>%
 ...
 kable_styling(latex_options="scale_down") %>%  
  kable_styling(latex_options = c("basic", "condensed"), full_width = FALSE) %>%
  row_spec(0,bold=TRUE) %>%
  save_kable(file = "tables/variation_of_nutrition_effects.pdf", keep_tex = TRUE)

#two_level_effects_table.df

# # TODO: Latex table testing with general effect only
# kable(two_level_effects_table.df[c(3:5)], "latex", escape = F, booktabs = T, row.names = FALSE, digits = 2, align="lr",
#   caption = "Variation of nutrition effects between different dialysis treatments and between patients within the same treatment.",
#   col.names = c("Nutrient", "Concentration", "General effect")) %>%
#   column_spec(1, width = "7em") %>%
#  kable_styling(latex_options="scale_down") %>%  
#   kable_styling(latex_options = c("basic", "condensed"), full_width = FALSE) %>%
#   row_spec(0,bold=TRUE) %>%
#   save_kable(file = "tables/variation_test_table.pdf", keep_tex = TRUE)
# two_level_effects_table.df
...
Here we query recommendations with graphical models. In recommendation, we let potassium and phosphorous intakes have values uniformly from full generally recommended range.

```{r recommendation_overview_simulation, eval=FALSE, cache=TRUE,echo=FALSE, message=FALSE}
source("mebn/v2/MEBNv2.r")
library(igraph)
library(gridExtra)
library(dplyr)
...
for (person_id in 1:37) {
  ...
  if (FALSE) {
    ...
    repeats <- rstan::extract(intake_model)
    ...
    personal_repeats <- cbind(median(pk),
                               median(true_concs$pk),
                               median(fppi),
                               median(true_concs$fppi),
                               median(palb),
                               median(true_concs$palb))
  } else {
  ...
  {
    saveRDS(accepted_intake.df, paste0("patient_summary/simulation",simulation,"/details/accepted_intake_",person_id,".rds"))
    ...
    for (recommandation_probability in recommandation_probability_levels)
    {
     ...
     if (nrow(recommendation_intake.df) > 0)
     {
        ...
        saveRDS(concs_minmax.df, paste0("patient_summary/simulation",simulation,"/details/concs_minmax_",person_id,".rds"))
  ...
  saveRDS(personal_recommendation, paste0("patient_summary/simulation",simulation,"/overviews/personal_recommendation_",person_id,".rds"))
} 

saveRDS(patient_summary, paste0("patient_summary/simulation",simulation,"/recommendation_summary_posterior_params.rds"))

```

## Recommendation plot

```{r personal_effect_summary, cache=TRUE,echo=FALSE}
library(igraph)
graph_dir <- "graphs/mv3_cross_two_levels/"
...
```{r concentration_highs_and_lows, echo=FALSE}
...
recom_summary <- readRDS(paste0("patient_summary/simulation",simulation,"/recommendation_summary_posterior_params.rds"))
...
for (person_id in 1:37) {
  ...
  personal_recommendation <- readRDS(paste0("patient_summary/simulation",simulation,"/overviews/personal_recommendation_",person_id,".rds"))
  
  concs_file <- paste0("patient_summary/simulation",simulation,"/details/concs_minmax_",person_id,".rds")
...
```{r reasoning_plot_ridges, fig.height=8.5, fig.width=8, cache=TRUE,echo=FALSE, message=FALSE}
...
for (person_id in 1:37) {
  ...
  personal_intake_file <- paste0("./patient_summary/simulation",simulation,"/details/accepted_intake_",person_id,".rds")
  ...
  if (file.exists(personal_intake_file)) {
    ...
    if (nrow(recommendations.df) > 0)
    {
      ...
      accepted_intake.df <- rbind(accepted_intake.df, c(person_id,0,0,0,0,0,0,0,0))
    }
...
pot_plot <- ggplot(intake_reasoning.df, aes(x = pot, y = reorder(person_text, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000), group = person_id)) +
      geom_density_ridges(aes(rel_min_height=0.01, scale = 1.5), fill = "#00BFC4", color = "#009FA4") +
      theme_bw() +
...
pho_plot <- ggplot(intake_reasoning.df, aes(x = pho, y = reorder(person_id, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000), group = person_id)) +
      ...
      geom_text(stat = "unique", aes(x=1000, label=pho_text), size=2.5, width=0.6) +
...
pk_plot <- ggplot(data = conc_reasoning.df, aes(y = reorder(person_id, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000)), group = accepted) +
      ...
      #geom_vline(aes(xintercept = pk_lower_limit), linetype="solid", color = "black", size=0.2, alpha=0.8) +
      #geom_vline(aes(xintercept = pk_upper_limit), linetype="solid", color = "black", size=0.2, alpha=0.8) +
...
fppi_plot <- ggplot(data = conc_reasoning.df, aes(y = reorder(person_id, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000)), group = accepted) +
      ...
      #geom_vline(aes(xintercept = fppi_lower_limit), linetype="solid", color = "black", size=0.2, alpha=0.8) +
      #geom_vline(aes(xintercept = fppi_upper_limit), linetype="solid", color = "black", size=0.2, alpha=0.8) +
...
palb_plot <- ggplot(data = conc_reasoning.df, aes(y = reorder(person_id, max_recom_norm + as.numeric(accepted_recommandation_probability) * 100000)), group = accepted) +
      ...
      #geom_vline(aes(xintercept = palb_lower_limit), linetype="solid", color = "black", size=0.2, alpha=0.8) +
      #geom_vline(aes(xintercept = palb_upper_limit), linetype="solid", color = "black", size=0.2, alpha=0.8) +
      #geom_vline(xintercept = c(36,45), linetype="solid", color = "black", size=0.2, alpha=0.8) +
...
```{r nuts_intake_method_figure, cache=TRUE}
library(ggplot2)
library(gridExtra)
library(ggrepel)

# Plot the personal recommendations
for (person_id in 10:10) {
...
#  person_id <- 28
  ...
  accepted_intake <- paste0("./patient_summary/simulation",simulation,"/details/accepted_intake_",person_id,".rds")
  ...
  if (file.exists(accepted_intake)) {
    ...
    {
      concs_minmax.df <- readRDS(paste0("patient_summary/simulation",simulation,"/details/concs_minmax_",person_id,".rds"))
    }
    
    rec_file <- paste0("patient_summary/simulation",simulation,"/overviews/personal_recommendation_",person_id,".rds")
    ...
    else
    {
      ...
      intake_plot <- ggplot(subset(accepted_intake.df, lowest_conc_prob >= 0.50), aes(x = pot, y = pho)) +
        geom_point(aes(colour=lowest_conc_prob), show.legend = TRUE) +
        geom_rect(fill = "#FFFFFF00", color = "white", size = 0.4,
        ...
        geom_rect(fill = MyPalette[1], color = MyPalette[1], size = 0.4,
                    aes(xmin = as.numeric(personal_recommendation.df$min_pot),
                    ymin = as.numeric(personal_recommendation.df$min_pho),
                    xmax = as.numeric(personal_recommendation.df$max_pot),
                    ymax = as.numeric(personal_recommendation.df$min_pho))) +
        geom_rect(fill = MyPalette[2], color = MyPalette[2], size = 0.4,
                    aes(xmin = as.numeric(personal_recommendation.df$min_pot),
                    ymin = as.numeric(personal_recommendation.df$max_pho),
                    xmax = as.numeric(personal_recommendation.df$max_pot),
                    ymax = as.numeric(personal_recommendation.df$max_pho))) +
        geom_rect(fill = MyPalette[3], color = MyPalette[3], size = 0.4,
                    aes(xmin = as.numeric(personal_recommendation.df$min_pot),
                    ymin = as.numeric(personal_recommendation.df$min_pho),
                    xmax = as.numeric(personal_recommendation.df$min_pot),
                    ymax = as.numeric(personal_recommendation.df$max_pho))) +
        geom_rect(fill = MyPalette[4], color = MyPalette[4], size = 0.4,
      ...
      pk_plot <- ggplot(concs_minmax.df, aes(x = pk, group=factor(color_order))) +
        ...
        scale_x_continuous(breaks=c(pk_recommendations,3,4,5,6)) +
        scale_colour_manual(values = MyPalette) +
        annotate("rect", xmin=-Inf, xmax=c(3.4), ymax=Inf, ymin=-Inf, alpha = .2) + 
        annotate("rect", xmin=c(4.7), xmax=Inf, ymax=Inf, ymin=-Inf, alpha = .2) + 
        theme_bw() +
      ...
      fppi_plot <- ggplot(concs_minmax.df, aes(x = fppi, group=factor(color_order))) +
        ...
        scale_x_continuous(breaks=c(fppi_recommendations,c(0,1,2,3))) +
        scale_colour_manual(values = MyPalette) +
        annotate("rect", xmin=c(1.8), xmax=Inf, ymax=Inf, ymin=-Inf, alpha = .2) + 
        theme_bw() +
        theme(axis.text.x = element_text(size=6), axis.title = element_text(size=8), plot.title = element_text(size=9))
      ...
      palb_plot <- ggplot(concs_minmax.df, aes(x = palb, color=factor(color_order), group=factor(color_order))) +
        ...
        scale_x_continuous(breaks=c(palb_recommendations,30,40,50)) +
        scale_colour_manual(values = MyPalette) +
        annotate("rect", xmin=-Inf, xmax=c(34), ymax=Inf, ymin=-Inf, alpha = .2) + 
        annotate("rect", xmin=c(48), xmax=Inf, ymax=Inf, ymin=-Inf, alpha = .2) + 
        theme_bw() +
