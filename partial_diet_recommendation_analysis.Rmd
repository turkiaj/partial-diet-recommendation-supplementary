...
fig_pufaepros <- ggplot() +
  geom_point(data=dialysis, aes(y = palb, x=pufaepros, color=potilas), size=0.4, show.legend = FALSE) +
  geom_line(data=dialysis, aes(y = palb, x=pufaepros, color=potilas), size=0.4, show.legend = FALSE)
...
kable(mse_table, "latex", escape = T, booktabs = T, row.names = FALSE, digits = 3,align="lrrrr",
  caption = "Supplementary Table S1: Comparison of model candidates. Personal graphical models with different reaction model candidates are evaluated with NRMSE for each modeled concentration and average model error.",
...
graph_dir <- "graphs/mv3_cross_two_levels/with_mode/"
...
saveRDS(effect_summary, "patient_summary/two_level_effect_summary_with_mode.rds")
...
effect_summary <- readRDS("patient_summary/two_level_effect_summary_with_mode.rds")
...
```{r plot_effect_density,eval=FALSE}
library(rstan)
library(ggplot2)

# TODO: Make an easily parameterized inspection tool
...
protgkg <- 13  # assumedpredictors_fat_epros$Name index
fosfori <- 17

palb <- 3

beta <- rstan::extract(twolvl_fit, pars = c(paste0("beta[",palb,",",fosfori,"]")))
group_effect <- rstan::extract(twolvl_fit, pars = c(paste0("group_effect[",group_id,",",palb,",",fosfori,"]")))
personal_effect <- rstan::extract(twolvl_fit, pars = c(paste0("personal_effect[",person_id,",",palb,",",fosfori,"]")))

dens1 <- as.data.frame(beta$`beta[3,17]`)
colnames(dens1) <- c("fosfori-palb")
dens1$level <- "general"

dens2 <- as.data.frame(group_effect$`group_effect[3,3,17]`)
colnames(dens2) <- c("fosfori-palb")
dens2$level <- "PD_avg"

dens3 <- as.data.frame(personal_effect$`personal_effect[10,3,17]`)
colnames(dens3) <- c("fosfori-palb")
dens3$level <- "personal"

dens <- rbind(dens1,dens2,dens3)

m1 <- mean(dens1$`fosfori-palb`) # -3.36
m2 <- median(dens1$`fosfori-palb`) # -1.96
...
m3 <- getmode(dens1$`fosfori-palb`) # 1.84

ggplot(dens, aes(x=`fosfori-palb`, fill=level)) +
...
\newpage
Supplementary Table S5: Table shows 20 effects that have highest variation between patients by comparing minimum and maximum of effect strength expected values.
```{r top_effect_summary_table,echo=FALSE, cache=TRUE}
...
# Query parameters
simulation <- "X50_B50_A50_I50_twolevel"
...
```{r reasoning_plot_ridges, fig.height=8.5, fig.width=8, cache=TRUE,echo=FALSE, message=FALSE, warning=FALSE,fig.cap="The figure shows in two left-most panels personal recommendations of potassium and phosphorous intake ($\\hat{Q}^{min}-\\hat{Q}^{max}$) from Algorithm (1) with predictive distributions of $\\hat{Q}$ in the background. Each row gives a numeric label of the patient and the type of patient's dialysis treatment (HD = hospital hemodialysis, HHD = home hemodialysis, PD = peritoneal dialysis). Personal recommendations are given for those patients whose resulting plasma concentration levels could be predicted in either over 90\\% or 80\\% probability. Three right-most panels show the estimated concentrations that match the recommended intake. In these panels, grey bars indicate the estimated concentration without the effect of potassium and phosphorous intake, $\\mu_{q0}$. Blue bars indicate the range of concentration that is resulted by modifying potassium and phosphorous intake. It is required that these simulated concentrations stay within the personal target ranges denoted with vertical black lines. The light blue bar indicates satisfying this requirement in $P^{max} > 80\\%$ confidence and dark blue $P^{max} > 90\\%$ confidence. The best exact probabilities $P^{max}_m$ are shown with percentage figures. Red bars indicate that even the best concentration estimation has $P^{max} < 80\\%$ confidence and personal recommendations are not given for these patients. For these patients, the red and blue bars show the whole reachable ranges. The figure is plotted with ggplot2 package for R language (v 3.3.5, https://ggplot2.tidyverse.org)"}
