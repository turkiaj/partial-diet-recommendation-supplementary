...
The analyzed dataset consists of food records and laboratory measurements from end-stage renal patients in dialysis. Following nutrients are possible considered as predictors of concentrations. Note that we use energy-% of fats and protein as unit in our analysis. This is referred as Table 1 in the main article.
...
```
and also following personal details and medication are considered as predictors. This is referenced as Table 2 in the article.
...
```
Aim of this analysis is to find such personal levels of potassium and phosphorous intakes that keep all these concentrations in their normal levels marked with white regions, if possible. 
...
For prediction, estimate the model without missing values in P-Alb. The rows with missing values are held out from the density estimation, but patients are kept in the model for estimating their parameters.
...
```{r, effect_correlation_plot_preparation, fig.width = 8, out.width="80%", fig.align="left", eval=FALSE, echo=FALSE}
library(rstan)
library(ggcorrplot)
...
```{r, effect_correlation_plot, fig.width=8, fig.out="100%", fig.align="left", eval=TRUE, cache=FALSE, echo=FALSE}
library(ggcorrplot)
...
```{r, top_correlations, eval=TRUE, cache=TRUE, echo=FALSE}
...
kable(top_correlations, "latex", escape = T, booktabs = T, row.names = FALSE, digits = 3, caption = "Table shows 30 highest positive or negative correlations between personal effects of nutrients or other considered predictors. This structure of correlations is used in estimating the personal effects based on personal intake and matching concentrations.") %>%
    kable_styling(latex_options = "HOLD_position")
...
```{r effect_summary_table, eval=FALSE, cache=TRUE, echo=FALSE}
library(dplyr)
...
```{r pot_pho_effects_only, cache=TRUE, echo=FALSE, eval=FALSE}
...
```{r nuts_intake_method_figure, cache=TRUE, eval=TRUE, echo=FALSE}
library(ggplot2)
library(gridExtra)
library(ggrepel)
