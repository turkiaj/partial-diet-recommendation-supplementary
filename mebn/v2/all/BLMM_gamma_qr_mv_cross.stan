...
transformed parameters {
  ...
  vector[v*k] b_stack[J];           // personal effects for each patient for all predictors at all responses
    ...
    for (m in 1:v)
    {
      for (n in 1:N-NH)
      {
        // - row n is picked from ZM_t and multiplied with column of coefs from a patient
         mu[vn] = mu[vn] + ZM_t[vn] * b_stack[group[n]];
         vn += 1;
      }
    }
...
generated quantities {
  ...
  // Unstack Y_rep to separate columns
  {
    ...
    for (m in 1:v)
    {
      ...
      {
        // - row n is picked from ZM_t and multiplied with column of coefs from a patient
         mu_hat[vn] = mu_hat[vn] + ZM_t[vn] * b_stack[group[n]];
         vn += 1;
      }
    }
    ...
    for (m in 1:v)
    {
      ...
      for (j in 1:J) 
      {
        ...
        // personal intercept
        personal_intercept[j,m] = beta_Intercept[m] + b[j,m][1];
    
        // beta vector does not include intercept, b is also sliced not to include it
        personal_effect[j,m] = beta[m] + b[j,m][2:k];
      }
    }
  }
}
