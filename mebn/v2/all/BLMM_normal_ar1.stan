...
model { 
  real mu;                // local variable for mean of Normal distribution
  ...
  for (n in 1:N) 
  {
     mu = temp_Intercept + Xc[n] * beta + Z[n] * b[group[n]]; 
...
generated quantities { 
  ...
  for (n in 1:N) 
  {
    ...
    Y_rep[n] = normal_rng(mu_hat, sigma_e);
    
    // Compute log-Likelihood for later LOO comparison of the models 
    log_lik[n] = normal_lpdf(Y[n] | mu_hat, sigma_e);
  }
