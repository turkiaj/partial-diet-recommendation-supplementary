...
mebn.multivariate_dens_overlays <- function(modelfile_path, target_variables, dataset)
{
  ...
  for (targetname in target_variables$Name)
  {
    ...
    posterior <- rstan::extract(target_blmm, pars = c("Y_rep"))
    posterior_y_50 <- posterior$Y_rep[3600:3700,i,]
...
mebn.extract_personal_graph_from_mv_lognormal <- function(person_id, reaction_graph, personal_model_dir, predictor_columns, target_variables, multivariate_modeldir, normalized_personal_data, original_personal_data, original_personal_concentrations, datadesc)
{
  library(igraph)
  library(rstan)
  ...
  for (c in 1:dim(target_variables)[1])
  {
    ...
    for (p in 1:length(predictor_names))
    {
      ...
      {
        org_mean <- mean(log(original_personal_data[,p]))
        org_sd <- sd(log(original_personal_data[,p]))
        norm_mean <- mean(log(normalized_personal_data[,p]))
        norm_sd <- sd(log(normalized_personal_data[,p]))
...
mebn.Query <- function(reaction_graph, graph_dir, query, queried_nodes, proposal_limits, conc_lower_limits, conc_upper_limits, stan_model_file, X_point_est = "mean", beta_point_est = "mean", param_point_est = "mean", posterior_samples = 100, X_sd_coef, repeat_only = 0, condition_in_repeat = 0)
{
  ...
  query_result <- stan(file=stan_model_file, warmup = 1000, iter=2000, chains=4, chain_id=1L, algorithm="NUTS", control = list(adapt_delta = 0.99, max_treedepth = 15), data=params, seed=483892929)
